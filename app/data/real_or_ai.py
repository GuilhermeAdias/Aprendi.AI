...
O jogador lê uma situação (com uma ilustração temática) e adivinha se aquilo
foi (provavelmente) feito por um ser humano ou gerado por Inteligência
Artificial. O objetivo é treinar o pensamento crítico e a percepção sobre
conteúdo gerado por IA.

Cada item tem:
- ``conteudo``: a descrição da situação;
- ``ilustracao``: um emoji que ilustra a cena (não entrega a resposta);
